...
wd = r'N:\temp\temp_akpona\x_calieco\06_predictions'
folder = 'landsat_single_month' # 'enmap_su'

for level in ["level1_lab","level2_lab","level3_lab","level4_lab"]:
    for descr in ['06']: # '06', 'enmap_su'
        print(descr)

        # pred_path = r'{0}\{1}\{2}_{3}\{2}_{3}_predictions_concatenated_{4}.tif'.format(wd, folder, descr, sub, level)
        pred_path = r'{0}\{1}\{2}\landsat_2013{2}15_bap_{3}\landsat_2013{2}15_bap_{3}_predictions_concatenated_{4}.tif'.format(wd, folder, descr,sub, level)
        ref_path = r'N:\temp\temp_akpona\x_calieco\05_ref_img\{0}\{0}_refimg_concatenated_{1}.tif'.format(sub, level)

        # out_path = r'N:\temp\temp_akpona\x_calieco\07_accuracy_assessment\{0}\{1}\{1}_{2}_{3}'.format(folder, descr, sub, level)
        # csv_path = r'N:\temp\temp_akpona\x_calieco\07_accuracy_assessment\{0}\{1}\{1}_{2}_{3}_mean.csv'.format(folder, descr, sub, level)
        out_path = r'N:\temp\temp_akpona\x_calieco\07_accuracy_assessment\{0}\{1}\landsat_2013{1}15_bap_{2}_{3}'.format(folder, descr, sub, level)
        csv_path = r'N:\temp\temp_akpona\x_calieco\07_accuracy_assessment\{0}\{1}\landsat_2013{1}15_bap_{2}_{3}_mean.csv'.format(folder, descr, sub, level)
        ...
        if level == "level1_lab":
            ...
            print(
                "No dictionary defined in the script. You need to define a dictionary where the band numbers are assigned to a class!")
        ...
        for i in range(num_bands):
            ...
            with open(csv_path, "a") as f:
                f.write(class_dict[i] + "," + str(r_sq) + "," + str(slope) + "," + str(intercept) + "," + str(mae) + "," + str(rmse) + ", pixel-wise\n")
            ...
            # Write model parameters to csv
            with open(csv_path, "a") as f:
                f.write(class_dict[i] + "," + str(r_sq) + "," + str(slope) + "," + str(
